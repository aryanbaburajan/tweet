...
export function Tweet({
  ...
  return (
    <div {...cx(`flex flex-col p-4 pb-0 bg-black text-gray-200 w-full`)}>
...
export async function fetchTweetData(tweetPath: string) {
  const fetchFromApi = async (path: string) => {
    return await fetch(`https://api.fxtwitter.com/${path}`, {
      cache: "force-cache",
  ...
  const response = await fetchFromApi(tweetPath);
  let tweetData = (await response.json()).tweet;
